...
[![Version](https://img.shields.io/badge/Version-26.4.2-22C55E)](https://github.com/nightfall-yl/Trimmings/releases)
[![License](https://img.shields.io/badge/License-MIT-blue)](LICENSE)

Trimmings 提供两大核心功能，可独立使用，也可在同一页面组合：
...
3. 在 Obsidian 设置 → 社区插件中启用 **Trimmings**

### 从源码构建

```bash
git clone https://github.com/nightfall-yl/Trimmings.git
cd Trimmings
npm install
npm run build
```
...
要编辑已有代码块，将光标移入 `homeboard` 代码块，运行 `Edit Trimmings (homeboard) block at cursor`，保存后变更会自动回写到源代码块。
...
| 命令 | 说明 |
|------|------|
| `Insert Trimmings (homeboard) block` | 插入新的 `homeboard` 代码块 |
| `Open Trimmings Builder` | 打开可视化 Builder |
| `Edit Trimmings (homeboard) block at cursor` | 编辑光标处的 `homeboard` 代码块 |
| `新建热力图` | 创建新的贡献图 |

编辑区右键可从上下文菜单访问 **新增 Trimmings 组件**。阅读模式下 `homeboard` 和 `contributionGraph` 代码块旁均有浮动编辑按钮。
...
src/
├── main.ts                    # 插件入口
├── builderModal.ts            # Trimmings Homeboard Builder 模态框
