...
import { defaultCalendarSettings, ICalendarSettings } from "../settings";
import { getDateUIDFromFile } from "./utils";

function createDailyNotesStore() {
	let hasError = false;
	const store = writable<Record<string, TFile>>({} as Record<string, TFile>);
	return {
		reindex: () => {
			try {
				...
				store.set({} as Record<string, TFile>);
				hasError = true;
			}
		},
		...store,
