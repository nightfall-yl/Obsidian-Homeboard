// @ts-nocheck
import type { ContributionCellData, HeatmapConfig } from "heatmap/types";
import { mapBy, matchCellStyleRule } from "heatmap/util/utils";
import { BaseGraphRender } from "./graphRender";
import { distanceBeforeTheStartOfWeek } from "heatmap/util/dateUtils";
import {
	localizedMonthMapping,
	localizedWeekDayMapping,
} from "heatmap/i18/messages";
...
export class GitStyleTrackGraphRender extends BaseGraphRender {
	...
	render(root: HTMLElement, graphConfig: HeatmapConfig): void {
		...
		for (let i = 0; i < contributionData.length; i++) {
			...
			if (item.value === 0) {
				if (item.date !== "$HOLE$") {
					// Empty day: leave background-color to CSS (.cell.empty) so it respects theme.
					// matchCellStyleRule(0) already returns null (explicitly handled).
					htmlParts.push(
					...
					// Hole cell (before start of week): invisible placeholder.
					htmlParts.push("<div class='cell'></div>");
				}
			} else {
				const rule = matchCellStyleRule(item.value, cellRules);
				// Fallback color: if no rule matches (shouldn't happen now), use the darkest
				// default rule instead of transparent. ALWAYS ensure a valid bg color.
				const fallbackRule = cellRules[cellRules.length - 1] ?? cellRules[0];
				const bg = rule?.color ?? fallbackRule?.color ?? "#216e39";
