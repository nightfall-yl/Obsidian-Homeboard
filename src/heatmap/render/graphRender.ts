...
import { parseDate } from "heatmap/util/dateUtils";
import {
	generateByLatestDays,
	generateByFixedDate,
	generateByData,
} from "./matrixDataGenerator";
import { matchCellStyleRule } from "heatmap/util/utils";
import { Locals } from "heatmap/i18/messages";
...
export abstract class BaseGraphRender implements GraphRender {
	...
	bindMonthTips(
		...
		// tips event
		monthCell.ariaLabel = `${String(yearMonthValue)} contributions on ${yearMonth}.`;
	}
	...
	applyCellGlobalStyleToContainer(
		...
		for (const key in cs) {
			const val: unknown = cs[key as keyof CSSStyleDeclaration];
			if (val != null && val !== "") {
